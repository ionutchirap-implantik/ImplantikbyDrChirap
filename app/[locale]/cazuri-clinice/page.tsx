import { notFound } from "next/navigation";
import { ScaffoldPage } from "@/components/shared/scaffold-page";
import { getDictionary } from "@/lib/i18n/get-dictionary";
import { createScaffoldMetadata } from "@/lib/create-scaffold-metadata";
import { PLACEHOLDERS } from "@/lib/constants";
import { isValidLocale, type Locale } from "@/lib/i18n/config";

type PageProps = { params: Promise<{ locale: string }> };

export async function generateMetadata({ params }: PageProps) {
  const { locale: localeParam } = await params;
  if (!isValidLocale(localeParam)) return {};
  return createScaffoldMetadata(
    localeParam as Locale,
    "cazuri-clinice",
    "Cazuri clinice",
    "Clinical cases",
    "Cazuistică proprie, cu consimțământul pacienților.",
    "Own case studies, with patient consent."
  );
}

export default async function Page({ params }: PageProps) {
  const { locale: localeParam } = await params;
  if (!isValidLocale(localeParam)) notFound();
  const locale = localeParam as Locale;
  const dict = await getDictionary(locale);
  const title = locale === "ro" ? "Cazuri clinice" : "Clinical cases";
  const desc = `${locale === "ro" ? "Before/after" : "Before/after"}: ${PLACEHOLDERS.beforeAfter}`;
  return <ScaffoldPage title={title} description={desc} dict={dict} locale={locale} />;
}
